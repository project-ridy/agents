...
6. **개발 기획서 작성** — `plans/` 디렉토리에 기획서 작성
7. **PR 생성** — 기획서 리뷰 요청
...
2. **이슈에 자신을 어사인** — `gh issue edit <번호> --add-assignee @me`
3. **기획서 로드** — `plans/`의 해당 기획서를 읽고 화면/컴포넌트/훅/API 구조 파악
...
| 책임 | 설명 |
|---|---|
| API 구현 | docs/api 스펙 기반으로 컨트롤러/서비스/DTO 구현 |
...
2. **이슈에 자신을 어사인** — `gh issue edit <번호> --add-assignee @me`
3. **기획서 로드** — `plans/`의 해당 기획서를 읽고 모듈/컨트롤러/서비스/DTO/Prisma 구조 파악
...
| 사용자 요청 분석 & 기획 | Planner | 개발 기획서 작성 (plans/) |
| 예외/엣지 케이스 정의 | Planner | 기획서 내 예외 처리 테이블 |
| 테스트 시나리오 설계 | Planner | 기획서 내 UT/IT/E2E 시나리오 |
| 백엔드 API 구현 | Backend Developer | NestJS 컨트롤러/서비스 |
