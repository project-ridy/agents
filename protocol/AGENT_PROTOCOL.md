...
Ridy 프로젝트는 5개의 에이전트가 협업합니다.
Hermes 프로필별 실제 모델/provider 설정은 `config/HERMES_MODELS.yaml`을 기준으로 관리합니다:
...
6. 담당 에이전트가 이슈에 자신을 어사인 (`gh issue edit <번호> --add-assignee @me`)
7. 이슈 상태를 "In Progress"로 변경
8. 담당 에이전트가 기획서 + docs 문서를 읽고 작업 수행
9. 브랜치 생성 → TDD 사이클 → PR 생성
10. PR 머지 후 이슈를 "Done"으로 변경
11. 기능 작업인 경우 후속 테스트 이슈를 "In Progress"로 변경
12. Orchestrator가 결과 확인 후 다음 작업 판단
```
...
3. 작업 시작 전 반드시 관련 docs 문서와 plans 기획서를 읽는다
...
2. `docs/design/SCREENS.md`, `docs/design/WIREFRAMES.md`를 기준으로 화면을 구현한다
...
- 에이전트 간 직접 소통 없음 — 모든 정보는 docs + agents + GitHub Project를 통해
- 질문/이슈는 GitHub 이슈에 BLOCKED 라벨 + 코멘트로 표현
- Orchestrator가 BLOCKED 이슈를 확인하고 docs/agents 업데이트로 해결
